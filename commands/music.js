function parseCommand(content) {
  ...
  const match = text.match(/^([!.])(play|radio|stop|skip|next|previous|pause|resume|queue)\b\s*(.*)$/i);
  ...
  handleMusicCommand: async (message, musicManager) => {
    ...
    try {
      ...
      if (parsed.command === 'radio') {
        if (!parsed.args) {
          await message.reply('Uso: `!radio <género o artista>`');
          return true;
        }
        if (!voiceChannel) {
          await message.reply('Debes estar en un canal de voz.');
        ...
        await musicManager.setRadioGenre(guildId, parsed.args);
        await musicManager.setRadioMode(guildId, true);
        const processingMsg = await message.reply(`Activando radio: **${parsed.args}**...`);
        const result = await musicManager.play(guildId, voiceChannel.id, `${parsed.args} mix`);
        await processingMsg.edit(`📻 Radio activa: **${result.title}** en el canal **${result.channel}**`);
        return true;
      }
